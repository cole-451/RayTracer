...
bool Plane::Raycast(const Ray& ray, const glm::vec3& point, const glm::vec3& normal, float minDistance, float maxDistance, float& t)
{
    ...
    t = glm::dot((point - ray.origin), normal / denominator); //(planePoint - rayOrigin) · planeNormal / denominator; is this a dot product?


        // return true if within distance bounds
        return (t > minDistance && t < maxDistance); // t is witnin > min distance and < max distance (t > minDistance && t < maxDistance)
}
