...
bool Metal::Scatter(const Ray& incident, const raycastHit_t& raycastHit, color3_t& attenuation, Ray& scattered) const {
    glm::vec3 reflected = glm::reflect(glm::normalize(incident.direction), raycastHit.normal);// <use glm reflect function with normalized incident direction and raycast hit normal>;
